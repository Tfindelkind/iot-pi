...
async def iot_send(telemetry_data):
    ...
    ################################################
    # Send telemetry 
    print ("Sending telemetry at "+ str(datetime.datetime.now()))
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            ## SELECT 
            #CAST("260" as float) as pv_dc_power1,  self.Adr[260] = [260,"Power DC1", "Float"]
            #CAST("270" as float) as pv_dc_power2,   self.Adr[270] = [270,"Power DC2", "Float"] 
            #CAST("106" as float) as own_consumption_from_battery,  self.Adr[106] = [106,"Home own consumption from battery", "Float"]
            #CAST("108" as float) as own_consumption_from_grid,            self.Adr[108] = [108,"Home own consumption from grid", "Float"]  
            #CAST("116" as float) as own_consumption_from_pv,    self.Adr[116] = [116,"Home own consumption from PV", "Float"]
            #CAST("194" as float) as battery_cycles,  self.Adr[194] = [194,"Number of battery cycles", "Float"]
            #CAST("200" as float) as actual_battery_charging,  self.Adr[200] = [200,"Actual battery charge -minus or discharge -plus current", "Float"]
            #CAST("216" as float) as battery_voltage,    self.Adr[216] = [216,"Battery voltage", "Float"]
            #CAST("514" as bigint) as battery_soc,  self.Adr[514] = [514,"Battery actual SOC", "U16"]
            #CAST("575" as bigint) as inverter_generation_power, self.Adr[575] = [575,"Inverter Generation Power (actual)", "S16"]
            #CAST("580" as bigint) as battery_net_capacity,  self.Adr[580] = [580,"Battery Net Capacity", "U32"]
            #CAST("582" as bigint) as actual_battery_charge_power, self.Adr[582] = [582,"Actual battery charge-discharge power", "S16"]
            #CAST(EventProcessedUtcTime as datetime) as timestamp
                #INTO
                #iopi
            #FROM
            #Kostal
           ...
           # battery_status["battery_status"] = Kostalquery.Qty['Battery actual SOC'][3]
